...
export async function readCustomerNote(jid: string) {
  ...
  const row = await db.from('whatsapp_lean_customers').where('jid', jid).first()
...
export async function writeCustomerNote(jid: string, note: string) {
  ...
  await db.rawQuery(
    `INSERT INTO whatsapp_lean_customers (jid, note, updated_at) VALUES (?, ?, ?)
...
export async function readOrderSpec(jid: string) {
  await ensureLeanTables()
  const row = await db.from('whatsapp_lean_specs').where('jid', jid).first()
...
export async function writeOrderSpec(jid: string, spec: string) {
  ...
  if (!value) {
    await db.from('whatsapp_lean_specs').where('jid', jid).delete()
    return ''
  }
  await db.rawQuery(
    `INSERT INTO whatsapp_lean_specs (jid, spec, updated_at) VALUES (?, ?, ?)
