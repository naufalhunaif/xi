...
export async function listLeanExamples(): Promise<LeanExample[]> {
  ...
  const rows = await db.from('whatsapp_lean_examples').where('active', 1).orderBy('id', 'asc')
  return rows.map((row) => ({
    id: Number(row.id),
...
export async function addLeanExample(example: LeanExample) {
  ...
  const [id] = await db.table('whatsapp_lean_examples').insert({
...
export async function removeLeanExample(id: number) {
  await ensureLeanTables()
  await db.from('whatsapp_lean_examples').where('id', id).update({ active: 0 })
}

/** Isi contoh bawaan sekali saja bila tabel masih kosong. */
export async function seedLeanExamples() {
  await ensureLeanTables()
  const existing = await db.from('whatsapp_lean_examples').count('* as total').first()
...
export async function syncSeedExamples() {
  ...
  const file = app.makePath('resources/lean/cs_examples.json')
  const list = JSON.parse(await readFile(file, 'utf8')) as LeanExample[]
  const rows = await db.from('whatsapp_lean_examples').select('customer_text', 'cs_text')
  const known = new Set(
