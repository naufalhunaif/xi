...
import {
  ...
  pickExamples,
  seedLeanExamples,
} from '#services/lean/lean_examples_service'
import {
  readCustomerNote,
  readOrderSpec,
  writeOrderSpec,
...
import {
  ...
  type LeanDecision,
  type LeanHistoryRow,
} from '#services/lean/lean_prompt'
...
import { readLeanState, writeLeanState } from '#services/lean/lean_tables'
 ...
 */
export const LEAN_SKILL_NAME = 'cs-inti'
...
export async function createLeanReply(input: {
  ...
  const [digest, examples, customerNote, contact, rows, spec] = await Promise.all([
    catalogDigest(),
    listLeanExamples(),
  ...
  const chatNote = contact?.chat_note ? String(contact.chat_note) : ''
  ...
  onTrace?.({ key: 'lean-ai', label: 'Menyusun balasan · tanpa tool', status: 'running' })
