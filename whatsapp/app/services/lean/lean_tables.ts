...
export const LEAN_TABLE_STATEMENTS = [
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_catalog (
    ...
    UNIQUE KEY whatsapp_lean_catalog_variant (product, color)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_examples (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_customers (
    jid VARCHAR(190) NOT NULL PRIMARY KEY,
    note TEXT NOT NULL,
    updated_at DATETIME NOT NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_orders (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    created_at DATETIME NOT NULL,
    updated_at DATETIME NOT NULL,
    KEY whatsapp_lean_orders_status (status, created_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_specs (
    jid VARCHAR(190) NOT NULL PRIMARY KEY,
    spec TEXT NOT NULL,
    updated_at DATETIME NOT NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  `ALTER TABLE whatsapp_lean_catalog
  ...
  `ALTER TABLE whatsapp_lean_orders
  ...
  `CREATE TABLE IF NOT EXISTS whatsapp_lean_state (
...
export async function readLeanState(name: string) {
  ...
  const row = await db.from('whatsapp_lean_state').where('name', name).first()
...
export async function writeLeanState(name: string, value: string) {
  await ensureLeanTables()
  await db.rawQuery(
    `INSERT INTO whatsapp_lean_state (name, value, updated_at) VALUES (?, ?, ?)
...
export async function deleteLeanChatData(trx: any, jids?: string[]) {
  await ensureLeanTables()
  for (const table of ['whatsapp_lean_customers', 'whatsapp_lean_specs', 'whatsapp_lean_orders']) {
    const query = trx.from(table)
    if (jids) query.whereIn('jid', jids)
    await query.delete()
  }
  const state = trx.from('whatsapp_lean_state')
  if (jids) {
    state.where((q: any) => {
