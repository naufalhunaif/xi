...
/**
 * Mode pengamat: chat yang dibalas CS manusia dirangkum diam-diam setelah sepi
 * beberapa menit, supaya order tetap tercatat. Tidak mengirim pesan apa pun.
 */
export async function observeHumanChats(limit = 2) {
  await ensureLeanTables()
  const quiet = new Date(Date.now() - 3 * 60_000)
  const since = new Date(Date.now() - 7 * 86_400_000)
  const result = await db.rawQuery(
    `SELECT m.jid, MAX(m.id) AS last_id, MAX(m.created_at) AS last_at
       FROM whatsapp_messages m
       LEFT JOIN whatsapp_contacts c ON c.jid = m.jid
      WHERE m.created_at >= ? AND (m.jid LIKE '%@s.whatsapp.net' OR m.jid LIKE '%@lid')
        AND COALESCE(c.ai_excluded, 0) = 0
      GROUP BY m.jid, c.handling_mode
     HAVING MAX(m.created_at) <= ?
        AND SUM(m.direction = 'in') > 0 AND COUNT(*) >= 4
        AND (c.handling_mode = 'cs'
          OR SUM(m.direction = 'out' AND m.sender_type IN ('cs', 'owner') AND m.created_at >= ?) > 0)
      ORDER BY MAX(m.created_at) DESC
      LIMIT 50`,
    [since, quiet, new Date(Date.now() - 86_400_000)]
  )
  const rows = (Array.isArray(result) ? result[0] : result) as Array<{ jid: string; last_id: number }>
  let handled = 0
  for (const row of rows) {
    if (handled >= limit) break
    const seen = await readLeanState(`amati:${row.jid}`)
    if (seen && Number(seen) >= Number(row.last_id)) continue
    await writeLeanState(`amati:${row.jid}`, String(row.last_id))
    handled++
    try {
      const recap = await summarizeChat(row.jid, since)
      if (recap) await saveRecap(row.jid, recap)
    } catch {}
  }
  return handled
}
