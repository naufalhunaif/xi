;(() => {
  ...
  function renderStatus(result) {
    ...
    byId('leanRoomHandling').textContent =
      handling.mode === 'cs'
    ...
    byId('leanRoomStage').textContent = [
    ...
    const rest = note
      ...
      .join('\n')
      .trim()
    byId('leanRoomChatNote').textContent = rest
    byId('leanRoomChatNote').hidden = !rest
  }

  function renderOrder(order) {
    const box = byId('leanRoomOrder')
    box.replaceChildren()
    byId('leanRoomOrderStatus').textContent = ''
    if (!order) {
      box.append(el('p', t('Belum ada form order dari pelanggan.'), 'wa-muted'))
      return
    }
    byId('leanRoomOrderStatus').textContent = [order.order_number || `#${order.id}`, statusLabel[order.status] || order.status, order.status === 'pending' && order.auto_total_reason ? t('Otomatis gagal: {0}', order.auto_total_reason) : '', groupLabel[order.group_status] || '']
      .filter(Boolean)
      .join(' · ')
    ...
    if (order.status === 'pending') {
      ...
      form.addEventListener('submit', async (event) => {
        ...
        try {
          await api(`/api/lean/orders/${order.id}/approve`, {
            itemsText: byId('leanRoomSpec').value || order.items,
      ...
      paid.addEventListener('click', async () => {
        try {
          const result = await api(`/api/lean/orders/${order.id}/paid`, {})
      ...
      resend.addEventListener('click', async () => {
        try {
          await api(`/api/lean/orders/${order.id}/resend-group`, {})
  ...
  async function load() {
    ...
    if (!jid) return
    try {
      const result = await api(`/api/lean/room?jid=${encodeURIComponent(jid)}`)
      if (result.jid !== jid) return
      current = result
      renderStatus(result)
      byId('leanRoomSpec').value = result.spec || ''
      byId('leanRoomNote').value = result.note || ''
      renderOrder(result.order)
    } catch (error) {
      notice(error.message, true)
    }
  }
  byId('leanRoomSpecSave').addEventListener('click', async () => {
    try {
      await api('/api/lean/room/spec', { jid, spec: byId('leanRoomSpec').value })
      notice(t('Detail pesanan disimpan.'))
    } catch (error) {
      notice(error.message, true)
    }
  })
  byId('leanRoomNoteSave').addEventListener('click', async () => {
    try {
      await api('/api/lean/customer', { jid, note: byId('leanRoomNote').value })
