;(() => {
  ...
  function render(data) {
    byId('igStatus').textContent = data.connected
      ? t('Terhubung sebagai @{0}', data.username || '—')
      : t('Belum terhubung')
    byId('igError').textContent = data.lastError ? `${t('Masalah terakhir')}: ${data.lastError}` : ''
    ...
    byId('igConnect').textContent = data.connected ? t('Hubungkan ulang') : t('Hubungkan Instagram')
  ...
  else if (result === 'failed') message(t('Gagal menghubungkan Instagram. Lihat masalah terakhir di atas.'))
  ...
  byId('igSave').addEventListener('click', () => save(credentials()))
  ...
  byId('igCommentTarget').addEventListener('change', (event) => {
    ...
    save({ commentTarget: event.target.value })
  })
  for (const toggle of switches)
    toggle.addEventListener('change', () => save({ [toggle.dataset.ig]: toggle.value === 'true' }))
})()
