...
import { catalogDigest, importLeanCatalog } from '#services/lean/lean_catalog_service'
import { estimateTokens } from '#services/prompt_size_service'
import { readLeanMcpConfig, syncLeanCatalog } from '#services/lean/lean_mcp'
...
export default class LeanCatalog extends BaseCommand {
  static commandName = 'lean:catalog'
  static description =
    ...
    return withActiveWorkspace(async () => {
      if (this.sync || !this.file) {
        ...
        if (!config.url || config.error) {
          this.logger.error(
            config.error || 'Sumber data belum dipilih. Jalankan lean:mcp --slug=… dulu.'
          )
          this.exitCode = 1
          return
        }
